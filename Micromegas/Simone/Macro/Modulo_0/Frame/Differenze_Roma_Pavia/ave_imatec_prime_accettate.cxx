...
void ave_imatec_prime_accettate (string scan){
...
//  scan.erase(scan.end()-4,scan.end());
 ...
 // reading the file
  ...
  while (1) {
    ...
    // actual reading
    in >> foil[j] >> aveR[j]>> aveP[j] >>  errR[j] >> errP[j]; 
...
//    errP[j] = errP[j]*3;
        ...
        errR[j]=errR[j]-aveR[j];
        errP[j]=aveR[j]-errP[j];

    cout << foil[j] <<"\t"<< aveR[j] <<"\t"<< aveP[j]<<"\t"<< errR[j] <<"\t"<< errP[j]<<"\t"<<endl; 
 ...
 // close the input file
  ...
  TGraph * g1 = new TGraphAsymmErrors(n, foil, aveR, errR, errP, errR, errP); 
...
*/
  ...
  mg->SetTitle("Frame Average Thickness; Frame number; Thickness (mm)");

  ostringstream Sdistrib;
  Sdistrib  << outpath << "/averageThickness_prime_accettate2.pdf";  
...
//  c0->Setgrid();
  ...
  pad2->Draw();
  TPaveLabel *title2 = new TPaveLabel(0.12,0.91,0.52,0.96,"Frame Average Thickness");
...
//  leg->Draw();
  ...
  TLine *l1 = new TLine(102.5,10.060,116.5,10.060);
  TLine *l2 = new TLine(102.5,9.940,116.5,9.940);
