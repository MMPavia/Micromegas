...
void prova_repeatability_and_mean (int p, int q, string scan1, string scan2, string scan3){
...
// file containing the data
  ...
  string nomefile=file1.str();
   ...
   TH2F *mean_map4 = new TH2F("mean_map4", "average Map4", nxbin, xmax, xmin, nybin, ymin, ymax );   //sono qui, metti a posto quel 30 e 60 ed aggiungi 
   mean_map4->GetXaxis()->SetTitle("X (mm)");                                                    //ed aggiungi TUTTO per gli altri 4 pcb
   mean_map4->GetYaxis()->SetTitle("Y (mm)");
   mean_map4->GetZaxis()->SetTitle("Z (#mum)");

   TH2F *mean_map3 = new TH2F("mean_map3", "average Map3", nxbin, xmax, xmin, nybin, ymin, ymax );   //sono qui, metti a posto quel 30 e 60 ed aggiungi 
   mean_map3->GetXaxis()->SetTitle("X (mm)");                                                    //ed aggiungi TUTTO per gli altri 4 pcb
   mean_map3->GetYaxis()->SetTitle("Y (mm)");
   mean_map3->GetZaxis()->SetTitle("Z (#mum)");

   TH2F *mean_map2 = new TH2F("mean_map2", "average Map2", nxbin, xmax, xmin, nybin, ymin, ymax );   //sono qui, metti a posto quel 30 e 60 ed aggiungi 
   mean_map2->GetXaxis()->SetTitle("X (mm)");                                                    //ed aggiungi TUTTO per gli altri 4 pcb
   mean_map2->GetYaxis()->SetTitle("Y (mm)");
   mean_map2->GetZaxis()->SetTitle("Z (#mum)");

   TH2F *mean_map1 = new TH2F("mean_map1", "average Map1", nxbin, xmax, xmin, nybin, ymin, ymax );   //sono qui, metti a posto quel 30 e 60 ed aggiungi 
   mean_map1->GetXaxis()->SetTitle("X (mm)");                                                    //ed aggiungi TUTTO per gli altri 4 pcb
...
// leggo file e riempio array con coord x e y
  ...
  ifstream in7 (file1.str().c_str());
  
   if(q==1){                    //gauge -->  x_gauge=x_laser+40
     for (int j=0; j<30; j++){
             in7 >> x >> y >> opt >> laser >>  temp1 >> temp2; // !!!!!!!!!!!!
             scan[8][j]=(x+40.0)/10;
             scan[6][j]=-y;
             //	     cout << scan[5][j] << " " << scan[6][j] << endl;
...
// leggo array e calcolo media e std di ogni punto
   for (int j=0; j<n; j++){
      ...
      mean_map5->Fill(scan[8][j],scan[6][j], mp5);
      cout << "X pcb5= " << scan[8][j] << "  y pcb5=  " << scan[6][j] << "  z pcb5= " << mp5 << endl;
      ...
      mean_map4->Fill(scan[8][j],scan[6][j], mp4);
      cout << "X pcb4= " << scan[8][j] << "  y pcb4=  " << scan[6][j] << "  z pcb4= " << mp4 << endl;
      ...
      mean_map3->Fill(scan[8][j],scan[6][j], mp3);
      cout << "X pcb3= " << scan[8][j] << "  y pcb3=  " << scan[6][j] << "  z pcb3= " << mp3 << endl;
      ...
      mean_map2->Fill(scan[8][j],scan[6][j], mp2);
      cout << "X pcb2= " << scan[8][j] << "  y pcb2=  " << scan[6][j] << "  z pcb2= " << mp2 << endl;
      ...
      mean_map1->Fill(scan[8][j],scan[6][j], mp1);
      cout << "X pcb1= " << scan[8][j] << "  y pcb1=  " << scan[6][j] << "  z pcb1= " << mp1 << endl;
