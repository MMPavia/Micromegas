...
void DIAG_diff_pcb_ref_stiffback (int p, string scan1){
...
// reference files
     if(p==1){
       file2 << inpath2 << "reference_stiffback_diag_int_sx.root";
     }
     else if(p==2){
       file2 << inpath2 << "reference_stiffback_diag_int_dx.root";
     }
     else if(p==3){
       file2 << inpath2 << "reference_stiffback_diag_ext_sx.root";
     }
     else if(p==4){
      file2 << inpath2 << "reference_stiffback_diag_ext_dx.root";
     }
   

   TFile *f1 = new TFile(file2.str().c_str());
   TH2F *ref_s1 = (TH2F*)f1->Get("map_s1");
   TH2F *ave_s1 = (TH2F*)ref_s1->Clone("reference");
   ...
   int ndistbin = 30; //cambiare se sono PCB o pannello
   float xdistmin =500; 
   float xdistmax =800; 
   ...
   Float_t x,y, opt, laser, press, temp2, coord;
    ...
    while(1){
	   ...
	   cout << x << " " << y << " " << opt << " "<< laser << " " <<  press << " " << temp2 << "c1=  " << coord << " "  << endl;
 ...
 // create a root file for the histograms;
  ...
  cout << "Root output file: " << endl << rootfile.str() << endl;
   ...
   map->SetStats(kFALSE);
   map->GetZaxis()->SetRangeUser(500, 800);
   map->Write();
   ...
   diff1->GetZaxis()->SetRangeUser(500, 800);
   diff1->Write();
