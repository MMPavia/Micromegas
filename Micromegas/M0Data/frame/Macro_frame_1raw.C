...
void Macro_frame(string scan1, string scan3="", string title= ""){
...
//    ifstream in6(file6.str().c_str());
    ...
    
    {
	    ...
	    cout<<"tast_val: "<<tast_val<<" opt_val: "<<opt_val<<endl;
  ...
  */  
    ...
    
    {
	    ...
	    cout<<"	ref opt_val "<<opt_val<<" tast_val: "<<tast_val<<endl;
...
//
		...
		//cout<<"alt0_up: "<<alt0_up<<""<<endl;
            
            
            
            tmap1->Fill(1, point, abs(alt0_up-alt0_ref));
            
//            std::cout << alt0_up-alt0_ref <<"\t" << alt1_up-alt1_ref << std::endl;

	    double diff = abs(alt0_up-alt0_ref);
	    cout<<"diff: "<<diff<<endl;
            frame1->Fill(diff);
	  ...
	  cout<<"pos_opt_ALL_UP[frame][0].size() "<<pos_opt_ALL_UP[frame][0].size()<<endl;
        for (int i=0; i<pos_opt_ALL_UP[frame][0].size();i++ ){
            ...
            up_0_vec.push_back(abs(abs(pos_opt_ALL_UP[frame][0][i]-pos_tast_ALL_UP[frame][0][i])-abs(pos_opt_ALL_REFERENCE[frame][0][i]-pos_tast_ALL_REFERENCE[frame][0][i])));
